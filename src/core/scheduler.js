...
export function evaluateEligibility(workload, offer, now = new Date()) {
  ...
  const regionAllowed = workload.allowedRegions.includes(offer.region) || (workload.allowedRegions.includes("EU") && EU_REGIONS.has(offer.region));
  if (!regionAllowed) reasons.push("region_not_allowed");
  if ((trustRank.get(offer.trustTier) ?? -1) < (trustRank.get(workload.minimumTrustTier) ?? Infinity)) reasons.push("trust_tier_too_low");
  if (!offer.dataClasses.includes(workload.dataClass)) reasons.push("data_class_not_allowed");
  if (offer.priceEur > workload.maximumPriceEur) reasons.push("price_exceeds_budget");
